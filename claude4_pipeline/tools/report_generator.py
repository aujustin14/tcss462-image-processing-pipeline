...
def report(responses, exp):
    ...
    for run in run_results:
        for key in runKeyList:
            if key not in run:
                print(
                    "PAYLOADS DO NOT CONTAIN EQUIVALENT ATTRIBUTES. MISSING ATTRIBUTES WILL BE FILLED WITH -999999999999 PURPOSEFULLY MAKING SUMS AND AVERAGES INCORRECT AS THEY CAN NO LONGER BE CALCULATED PROPERLY."
                )
    ...
    if "3_pipeline_stage" in run_results[0]:
        ...
        pipelineList = [
            [[None for col in range(maxZ + 1)] for col in range(maxY + 1)]
        ...
        for x in pipelineList:
            for y in x:
                y = sorted(
                    [item for item in y if item is not None],
                    key=lambda k: k["3_pipeline_stage"],
                )
                previous = None
                for z in y:
                    print(
    ...
    for i in range(len(run_results)):
        ...
        for j in range(len(invalidKeys)):
            ...
            if key_value in list(run.keys()) and str(run[key_value]) == str(
                invalidators[key_value]
            ):
                invalidRuns.append(run)

    if len(invalidRuns) > 0:
        output += "\n" + str(len(invalidRuns)) + " runs removed from categories....\n"
    ...
    if "zTenancy[vmID[iteration]]" in categories or "zTenancy[vmID]" in categories:
        ...
        for attribute in tenancyAttributes:
            ...
            if valid:
                for i in range(len(run_results)):
                    run = run_results[i]
                    run["zTenancy[" + attribute + "]"] = (
                        str(vm_dictionary[run[attribute]]["cpuType"])
                        + " - "
                        + str(vm_dictionary[run[attribute]]["uses"])
                    )
                    run["tenants[" + attribute + "]"] = vm_dictionary[run[attribute]][
                        "uses"
                    ]
                    if (
                        attribute == "vmID[iteration]"
                        and "zTenancy[vmID]" in categories
                    ):
    ...
    for i in range(len(master_key_list)):
        ...
        if len(sub_key_list) != 0:
            ...
            for i in range(len(run_attributes)):
                ...
                valid = True
                if (
                    attribute in ignore_attributes
                    or attribute in ignore_attributes_from_all_categories
                ):
                    valid = False

                if key_value in ignore_attributes_from_specific_categories:
                    if attribute in list(
                        ignore_attributes_from_specific_categories[key_value]
                    ):
                        valid = False
        ...
        if len(runList) > 0:
            ...
            if not os.path.exists(baseFileName):
                ...
                for i, run in enumerate(runList):
                    temp = uuid.uuid4()
                    file = open(
                        baseFileName + "/run" + str(i) + "-" + str(temp) + ".json", "w"
                    )
